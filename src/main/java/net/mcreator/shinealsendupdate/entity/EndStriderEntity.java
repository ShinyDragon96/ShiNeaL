...
public class EndStriderEntity extends Monster implements IAnimatable {
	...
	protected void defineSynchedData() {
		...
		this.entityData.define(TEXTURE, "unknown");
	}
	...
	protected void registerGoals() {
		...
		this.goalSelector.addGoal(1, new RandomStrollGoal(this, 1, 20) {
			...
			protected Vec3 getPosition() {
				RandomSource random = EndStriderEntity.this.getRandom();
				double dir_x = EndStriderEntity.this.getX() + ((random.nextFloat() * 2 - 1) * 16);
				double dir_y = EndStriderEntity.this.getY() + ((random.nextFloat() * 2 - 1) * 16);
				double dir_z = EndStriderEntity.this.getZ() + ((random.nextFloat() * 2 - 1) * 16);
		...
		this.goalSelector.addGoal(3, new Goal() {
			...
			public boolean canUse() {
				if (EndStriderEntity.this.getTarget() != null && !EndStriderEntity.this.getMoveControl().hasWanted()) {
					return true;
				} else {
					return false;
				}
			...
			public boolean canContinueToUse() {
				return EndStriderEntity.this.getMoveControl().hasWanted() && EndStriderEntity.this.getTarget() != null && EndStriderEntity.this.getTarget().isAlive();
			}

			@Override
			public void start() {
				LivingEntity livingentity = EndStriderEntity.this.getTarget();
				Vec3 vec3d = livingentity.getEyePosition(1);
				EndStriderEntity.this.moveControl.setWantedPosition(vec3d.x, vec3d.y, vec3d.z, 1);
			}

			@Override
			public void tick() {
				LivingEntity livingentity = EndStriderEntity.this.getTarget();
				if (EndStriderEntity.this.getBoundingBox().intersects(livingentity.getBoundingBox())) {
					EndStriderEntity.this.doHurtTarget(livingentity);
				} else {
					double d0 = EndStriderEntity.this.distanceToSqr(livingentity);
					if (d0 < 16) {
						Vec3 vec3d = livingentity.getEyePosition(1);
						EndStriderEntity.this.moveControl.setWantedPosition(vec3d.x, vec3d.y, vec3d.z, 1);
					}
	...
	public static void init() {
		SpawnPlacements.register(ShinealsEndUpdateModEntities.END_STRIDER.get(), SpawnPlacements.Type.ON_GROUND, Heightmap.Types.MOTION_BLOCKING_NO_LEAVES,
	...
	protected void tickDeath() {
		...
		if (this.deathTime == 20) {
			this.remove(EndStriderEntity.RemovalReason.KILLED);
			this.dropExperience();
		}
	}
